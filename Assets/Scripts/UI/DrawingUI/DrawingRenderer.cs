...
public class DrawingRenderer : MonoBehaviour
{
    ...
    private Texture2D m_drawingTexture, m_previewTexture;

    //Color array 
    Color[] m_drawingPixelColors, m_previewPixelColors;
    ...
    private void Awake()
    {
        ...
        m_renderTexture.width = m_textureSize.x;
        m_renderTexture.height = m_textureSize.y;
        //Ensures that we use sRGB so the colors are displayed correctly when you import our drawing to Aseprite or other sowftware
    ...
    void Start()
    {
        ...
        //Ensures that preview texture is the same size as RenderTexture
        m_previewRenderTexture.width = m_renderTexture.width;
        m_previewRenderTexture.height = m_renderTexture.height;
        //Color format is important for saving to png
        m_previewRenderTexture.format = m_renderTexture.format;
        m_previewTexture =
            new(m_previewRenderTexture.width, m_previewRenderTexture.height, TextureFormat.RGBA32, false);
        m_drawingTexture.filterMode = FilterMode.Point;
    ...
    {
        //Apply colors to our Texture2D
        textureToUpdate.SetPixels(pixelsToApply);
        //We need to call Apply to Texture2D before we print its content to the RenderTexture
    ...
    {
        DrawOnTexture(color, pixelPositions, m_renderTexture, m_drawingTexture, m_drawingPixelColors, blending);
    }
    ...
    /// <param name="blending"></param>
    private void DrawOnTexture(Color color, List<Vector2Int> pixelPositionsInTextureSpace, RenderTexture renderTexture, Texture2D textureToUpdate, Color[] colors, bool blending)
    {
        //apply brush size
    ...
    {
        ClearTexture(m_drawingPixelColors, m_renderTexture, m_drawingTexture);
    }
